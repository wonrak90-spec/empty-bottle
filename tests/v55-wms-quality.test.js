...
assert.ok(Q);
assert.strictEqual(Q.VERSION,'V55-WMS-QUALITY-1');
...
assert.strictEqual(Q.validateBeforeSave({
  ...
  vendorQty:'21320',
  expiryDate:'2031-09-17'
}).ok,true,'normal quantity + future expiry date must remain valid');
...
console.log('PASS V55 WMS quality V1: noisy-field sanitize + date/year/quantity guards');
