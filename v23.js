...
(function(){
  ...
  const STORE='emptyBottle.v23.session';
  const ADMIN_STORE='emptyBottle.v23.adminSession';

  /* ==================== 보안 세션 브리지 ==================== */
  function loadStored(key){
    try{const x=JSON.parse(localStorage.getItem(key)||'null');if(x&&x.session&&Number(x.expiresAt)>Date.now()+5000)return x;}catch(_){ }
    try{localStorage.removeItem(key);}catch(_){ } return null;
  }
  ...
  async function rawJson(url,opts){
    ...
    try{return JSON.parse(t);}catch(_){throw new Error('서버 응답을 해석할 수 없습니다. Apps Script 배포 URL을 확인하세요.');}
  }
  async function publicGet(action,params){
    const qs=new URLSearchParams(Object.assign({action},params||{}));return rawJson(CONFIG.API_URL+'?'+qs.toString());
  }
  async function publicPost(action,payload){
    return rawJson(CONFIG.API_URL,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action,payload:payload||{}})});
  }
  async function postWithSession(action,payload,session){
    return rawJson(CONFIG.API_URL,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action,payload:payload||{},session:session||''})});
  }

  function injectAuthUi(){
    if($('v23AuthOverlay'))return;
    const el=document.createElement('div');el.id='v23AuthOverlay';el.className='v23-auth hidden';
    el.innerHTML=`<div class="v23-auth-card">
      <div class="v23-auth-title">공병 입고 확인</div>
      <div class="v23-auth-sub">내부 사용자 인증</div>
      <div class="field"><label>접속코드</label><input id="v23AccessCode" type="password" autocomplete="off" placeholder="접속코드를 입력하세요"></div>
      <button id="v23AuthBtn" class="btn primary">접속</button>
      <div id="v23AuthStatus" class="status">작업자 접속코드는 브라우저 소스에 저장되지 않습니다.</div>
      <div class="v23-auth-version" id="v23AuthVersion">Frontend ${V23.VERSION}</div>
    </div>`;
    document.body.appendChild(el);
    $('v23AuthBtn').onclick=loginFromModal;
    $('v23AccessCode').addEventListener('keydown',e=>{if(e.key==='Enter')loginFromModal();});
  }
  function showAuth(message){
    injectAuthUi();$('v23AuthOverlay').classList.remove('hidden');
    if(message)$('v23AuthStatus').textContent=message;
    setTimeout(()=>$('v23AccessCode').focus(),50);
  }
  function hideAuth(){if($('v23AuthOverlay'))$('v23AuthOverlay').classList.add('hidden');}
  function finishAuth(session){
    V23.session=session;saveStored(STORE,session);hideAuth();
    if(V23.authResolve){V23.authResolve(session);V23.authResolve=null;V23.authReject=null;V23.authPromise=null;}
    setTimeout(()=>{try{if(typeof loadDashboard==='function')loadDashboard();}catch(_){}},50);
  }
  async function loginFromModal(){
    const code=$('v23AccessCode').value.trim();if(!code){$('v23AuthStatus').textContent='접속코드를 입력하세요.';return;}
    $('v23AuthBtn').disabled=true;$('v23AuthStatus').textContent='인증 중...';
    try{
      const res=await publicPost('authLogin',{code});
      if(!res.ok){$('v23AuthStatus').textContent=res.message||'인증 실패';return;}
      $('v23AccessCode').value='';finishAuth({session:res.session,role:res.role||'operator',expiresAt:res.expiresAt||Date.now()+21600000});
    }catch(e){$('v23AuthStatus').textContent='인증 실패: '+e.message;}
    finally{$('v23AuthBtn').disabled=false;}
  }
  async function ensureSession(){
    if(!V23.session)V23.session=loadStored(STORE);
    if(V23.session&&Number(V23.session.expiresAt)>Date.now()+5000)return V23.session;
    V23.session=null;clearStored(STORE);
    if(!V23.authPromise){V23.authPromise=new Promise((resolve,reject)=>{V23.authResolve=resolve;V23.authReject=reject;});showAuth('접속 인증이 필요합니다.');}
    return V23.authPromise;
  }
  function authExpired(message){V23.session=null;clearStored(STORE);showAuth(message||'접속 세션이 만료되었습니다. 다시 인증하세요.');}

  window.apiGet=async function(action,params){
    const sess=await ensureSession();
    const qs=new URLSearchParams(Object.assign({action,session:sess.session},params||{}));
    const res=await rawJson(CONFIG.API_URL+'?'+qs.toString());
    if(res&&res.code==='AUTH_REQUIRED'){authExpired(res.message);throw new Error(res.message||'인증 필요');}
    return res;
  };
  window.apiPost=async function(action,payload){
    const sess=await ensureSession();
    const res=await postWithSession(action,payload,sess.session);
    if(res&&res.code==='AUTH_REQUIRED'){authExpired(res.message);throw new Error(res.message||'인증 필요');}
    return res;
  };

  async function ensureAdminSession(){
    if(V23.session&&V23.session.role==='admin'&&Number(V23.session.expiresAt)>Date.now()+5000)return V23.session;
    if(!V23.adminSession)V23.adminSession=loadStored(ADMIN_STORE);
    if(V23.adminSession&&V23.adminSession.role==='admin'&&Number(V23.adminSession.expiresAt)>Date.now()+5000)return V23.adminSession;
    const code=prompt('기록 삭제는 관리자 인증이 필요합니다.\n관리자 접속코드를 입력하세요.','');if(code===null)return null;
    const res=await publicPost('authLogin',{code:code.trim()});
    if(!res.ok){alert(res.message||'관리자 인증 실패');return null;}
    if(res.role!=='admin'){alert('관리자 접속코드가 아닙니다.');return null;}
    V23.adminSession={session:res.session,role:'admin',expiresAt:res.expiresAt};saveStored(ADMIN_STORE,V23.adminSession);return V23.adminSession;
  }

  async function bootstrapSecurity(){
    injectAuthUi();
    try{
      const info=await publicGet('securityInfo');V23.backendInfo=info;
      if($('v23AuthVersion'))$('v23AuthVersion').textContent=`Frontend ${V23.VERSION} · Backend ${info.backendVersion||'?'} · DB ${info.schemaVersion||'?'}`;
      if(!info.securityEnabled){showAuth('Apps Script에서 setupSecurityV23()를 먼저 실행해야 합니다.');$('v23AuthBtn').disabled=true;return;}
      V23.session=loadStored(STORE);if(!V23.session)showAuth();else hideAuth();
    }catch(e){showAuth('백엔드 연결 확인 실패: '+e.message);}
  }

  /* ==================== 조회 목록 / 페이지 ==================== */
  function resetReportState(){
    V23.reportItems=null;V23.reportKind='';V23.reportPhotoData.clear();
  ...
  V23.resetReportState=resetReportState;
  ...
  function injectListControls(){
    ...
    ctl.innerHTML=`<div class="v23-list-left"><button class="btn outline" onclick="V23.loadRecords(1)">최근 등록 목록</button><span id="v23ListSummary">-</span><span id="v23SelectedCount">선택 0건</span></div>
    ...
    $('v23PageSize').onchange=()=>{V23.pageSize=Number($('v23PageSize').value)||30;V23.loadRecords(1);};
    const input=$('viewSearchKw');if(input){input.placeholder='입고번호 / 품명 / 품목코드 / 공급업체 / 검수자 (공란이면 전체)';}
  }
  function pageButtons(){
    const p=V23.page,t=V23.totalPages,arr=[];for(let n=Math.max(1,p-2);n<=Math.min(t,p+2);n++)arr.push(n);
    return `<button ${p<=1?'disabled':''} onclick="V23.loadRecords(${p-1})">‹ 이전</button>${arr.map(n=>`<button class="${n===p?'active':''}" onclick="V23.loadRecords(${n})">${n}</button>`).join('')}<button ${p>=t?'disabled':''} onclick="V23.loadRecords(${p+1})">다음 ›</button>`;
  }
  ...
  V23.loadRecords=async function(page){
    injectListControls();resetReportState();V23.page=Math.max(1,Number(page)||1);V23.pageSize=Number($('v23PageSize')&&$('v23PageSize').value)||V23.pageSize||30;V23.keyword=($('viewSearchKw')&&$('viewSearchKw').value||'').trim();
    ...
    try{
      const res=await apiGet('listRecords',{keyword:V23.keyword,page:V23.page,pageSize:V23.pageSize});if(!res.ok)throw new Error(res.message||'조회 실패');
      V23.page=res.page||1;V23.pageSize=res.pageSize||V23.pageSize;V23.total=res.total||0;V23.totalPages=res.totalPages||1;V23.listLoaded=true;
      ...
      $('v23ListSummary').textContent=`총 ${Number(V23.total).toLocaleString()}건 · ${V23.page}/${V23.totalPages}페이지`;
      $('v23Pager').innerHTML=pageButtons();updateSelectedCount();
      if(typeof setStatus==='function')setStatus('viewSearchStatus',V23.keyword?`“${V23.keyword}” 검색 ${V23.total}건`:`최근 등록순 ${V23.total}건`,'ok');
    }catch(e){if(typeof setStatus==='function')setStatus('viewSearchStatus','조회 실패: '+e.message,'bad');}
  };
  window.searchViewRecords=function(){return V23.loadRecords(1);};
  ...
  function patchDetail(){
    ...
    const fn=async function(i){resetReportState();const out=await old(i);setTimeout(enhanceDetail,0);return out;};fn.__v23=true;window.openRecordDetail=fn;
  }
  ...
  async function loadPhotoData(refs){
    const map=new Map(),errors={};for(let i=0;i<refs.length;i+=6){const part=refs.slice(i,i+6),urls=part.map(x=>x.url);const res=await apiPost('photoDataBatchV23',{urls});if(res&&res.items)Object.keys(res.items).forEach(u=>map.set(u,res.items[u]));if(res&&res.errors)Object.assign(errors,res.errors);}return {map,errors};
  }
  ...
  V23.closeReport=function(){resetReportState();};
  async function prepareReport(kind){
    ...
    try{const items=await batchRecords(ids);V23.reportItems=items;V23.reportKind=kind;if(window.V22){V22.reportData=items;V22.reportKind=kind;}buildReportForm(kind,items);const c=$('v22ReportBuilder');c.classList.remove('hidden');const actions=c.querySelector('.v22-action-row');if(actions)actions.innerHTML=`<button class="btn primary" onclick="V22.printReport()">🖨 보고서 출력</button><button class="btn outline" onclick="V23.closeReport()">닫기</button>`;c.scrollIntoView({behavior:'smooth',block:'start'});}catch(e){alert('보고서 준비 실패: '+e.message);}finally{if(typeof hideBusy==='function')hideBusy();}
  }
  ...
  async function printReport(){
    const items=V23.reportItems;if(!items||!items.length){alert('보고서를 다시 선택해서 준비하세요.');return;}
    const w=window.open('','_blank');if(!w){alert('팝업을 허용해주세요.');return;}w.document.write('<p>보고서와 증빙사진을 준비 중입니다...</p>');
    try{const refs=photoRefs(items,opt()),loaded=await loadPhotoData(refs);V23.reportPhotoData=loaded.map;const body=V23.reportKind==='executive'?executiveHtml(items,refs,loaded.map,loaded.errors):investigationHtml(items,refs,loaded.map,loaded.errors);const html=`<!doctype html><html lang="ko"><head><meta charset="utf-8"><title>${V23.reportKind==='executive'?'공병 입고 임원보고':'공병 입고 이상 조사보고서'}</title>${reportCss()}</head><body>${body}<script>window.onload=()=>setTimeout(()=>window.print(),500)<\/script></body></html>`;w.document.open();w.document.write(html);w.document.close();if(refs.length&&!loaded.map.size)alert('선택한 증빙사진을 불러오지 못했습니다. 보고서의 경고 문구와 Drive 파일 상태를 확인하세요.');}catch(e){w.document.body.innerHTML='<pre>'+esc(e.message||e)+'</pre>';}
  }
  ...
  async function deleteCurrent(){
    ...
    const actor=prompt('삭제자 이름을 입력하세요.',r.inspector||'');if(actor===null)return;const reason=prompt('삭제 사유를 입력하세요. (필수)','오등록');if(reason===null)return;if(!actor.trim()||!reason.trim()){alert('삭제자와 삭제 사유는 필수입니다.');return;}
    ...
    try{const res=await postWithSession('deleteRecord',{id:r.id,actor:actor.trim(),reason:reason.trim(),confirmText:'DELETE'},admin.session);if(!res.ok){alert('삭제 중단: '+(res.message||'알 수 없는 오류')+(res.code==='PHOTO_DELETE_PARTIAL'?'\nDB 기록은 유지되어 재시도할 수 있습니다.':''));return;}if(window.V22)V22.selectedIds.delete(String(r.id));currentViewRecord=null;if($('viewDetailCard'))$('viewDetailCard').classList.add('hidden');if($('v22EditCard'))$('v22EditCard').classList.add('hidden');resetReportState();alert(`삭제 완료\nDB 기록 삭제: 완료\nDrive 사진: ${res.deletedPhotos||0}/${res.photoCount||0}개\nAudit/삭제보관 ID: ${res.deletionId||'-'}`);await V23.loadRecords(V23.page);}catch(e){alert('삭제 실패: '+e.message);}finally{if(typeof hideBusy==='function')hideBusy();}
  }
  function patchDelete(){if(window.V22)V22.deleteCurrent=deleteCurrent;}

  /* ==================== 초기화 ==================== */
  function patchTools(){
    injectListControls();patchSelection();patchDetail();patchReport();patchDelete();
    ...
    const prev=window.showTab;if(typeof prev==='function'&&!prev.__v23){const fn=function(tab){const x=prev(tab);if(tab==='view'&&!V23.listLoaded)setTimeout(()=>V23.loadRecords(1),50);return x;};fn.__v23=true;window.showTab=fn;}
    const sub=document.querySelector('header .sub');if(sub)sub.textContent='단건 중심 · 보안세션 · Audit · 조회/보고 V22.3';
  }
