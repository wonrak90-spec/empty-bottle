/* V55 OCR Robustness Benchmark Extension V1
...
(function(){
  ...
  const B=window.V55OcrBenchmark={
    VERSION:'V55-OCR-ROBUSTNESS-BENCHMARK-1',
    running:false,
    results:[],
    summary:null,
    zipName:''
  ...
  function modal(){
    ...
    m.innerHTML='<div style="max-width:920px;margin:20px auto;background:#fff;border-radius:14px;padding:18px;color:#111">'+
      '<div style="display:flex;justify-content:space-between;gap:12px;align-items:center"><div><b style="font-size:18px">OCR Baseline Benchmark V1</b><div style="font-size:12px;color:#666;margin-top:3px">Seed ZIP은 브라우저 안에서만 처리되며 서버로 전송하지 않습니다.</div></div><button class="btn outline" onclick="V55OcrBenchmark.close()">닫기</button></div>'+
      ...
      '<div style="display:flex;gap:8px;margin-top:12px"><button id="v26BenchCsv" class="btn outline hidden" onclick="V55OcrBenchmark.downloadCsv()">결과 CSV</button></div>'+
      '</div>';
  ...
  function injectButton(){
    ...
    const bar=$('v24AccountBar');if(!bar)return;
    const actions=bar.querySelector('.v24-account-actions');if(!actions)return;
    let b=$('btnOcrBenchmark');
    if(!b){
      b=document.createElement('button');b.id='btnOcrBenchmark';
      actions.insertBefore(b,actions.firstChild);
    }
    b.textContent='OCR 품질';
    b.onclick=B.open;
    b.dataset.v55Benchmark='1';
  }
  function init(){
    injectButton();
    ...
    console.info('[V55-OCR-ROBUSTNESS-BENCHMARK-1] enhanced admin local benchmark active');
  }
